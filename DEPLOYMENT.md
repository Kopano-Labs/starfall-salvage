...
Use the Python backend when demoing sign-in and score sync:
...
The game can be hosted as a static site. Pilot profiles and scores still work locally through browser `localStorage`, but shared leaderboard sync will not exist unless the Python API is hosted separately.
...
- Decide whether the Python backend is local-demo only or production-hosted.
